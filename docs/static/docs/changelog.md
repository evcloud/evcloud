...
2024-06-14
* 增加公共镜像任务表
* 增加公共镜像任务接口：公共镜像上传、添加公共镜像任务信息、修改公共镜像任务信息、启动/暂停/删除公共镜像任务、获取公共镜像任务信息接口
* 全数据中心资源汇总接口：增加ips_public_used(公网已使用IP数)、ips_private_used(内网已使用IP数)
* 全数据中心资源汇总接口参数修改：vpn 在线数去重、查询近一个月的登录数据
* 镜像任务脚本：增加目录空间判断限制30G,小于30G则不下载。
* admin 页面增加IP限制
* 依赖包更新 requests 版本和中坤保持一致
