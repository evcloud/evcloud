...
def api_log(func):
    '''api 日志 ，使用logging模块将日志打入文件 by lzx 20180726'''
    def handle_args(*args, **kwargs): 
        try:
            start_time = datetime.datetime.now()
            res = func(*args, **kwargs)
            ...
            if len(args) > 0 and type(args[0]) == dict and 'req_user' in args[0]:
                try:
                    dic = {}
                    dic['user_id'] = args[0]['req_user'].id
                    dic['user_name'] = args[0]['req_user'].username
                    
                    dic['op'] = func.__module__ + '.' + func.__name__
                    dic['start_time'] = start_time
                    dic['finish_time'] = end_time
                    dic['result'] = res['res']
                    dic['from_trd_part'] = '__from_post' in args[0]
                    dic['req_args'] = args
                    if not dic['result']:
                        error = ''
                        if 'err' in res:
                        ...
                        dic['error'] = '-'
                    logger.info('',extra=dic)
                except Exception as e:
                    print('[api_log error]',e, res)
