...
class VmsViewSet(CustomGenericViewSet):
    ...
    def vm_vnc(self, request, *args, **kwargs):
        ...
        user_operation_record.add_log(request=request, operation_content=f'远程连接云主机vnc,云主机IP：{vm.mac_ip}',
                                      remark='')
        # 搁置虚拟机，不允许
        status_bool = vm_normal_status(vm=vm)
        if status_bool is False:
    ...
    def vm_remark(self, request, *args, **kwargs):
        ...
        api = VmAPI()
        try:
            api.modify_vm_remark(user=request.user, vm_uuid=vm_uuid, remark=remark, request=request)
        except VmError as e:
    ...
    def vm_snap_remark(self, request, *args, **kwargs):
        ...
        try:
            VmAPI().modify_sys_snap_remarks(snap_id=snap_id, remarks=remark, request=request)
        except VmError as e:
    ...
    def vm_reset(self, request, *args, **kwargs):
        ...
        api = VmAPI()
        try:
            api.change_sys_disk(vm_uuid=vm_uuid, image_id=image_id, request=request, user=request.user)
        except VmError as e:
    ...
    def vm_migrate(self, request, *args, **kwargs):
        ...
        api = VmAPI()
        try:
            api.migrate_vm(vm_uuid=vm_uuid, host_id=host_id, request=request, force=is_force)
        except VmError as e:
    ...
    def vm_migrate_live(self, request, *args, **kwargs):
        ...
        api = VmAPI()
        try:
            m_task = api.live_migrate_vm(vm_uuid=vm_uuid, dest_host_id=host_id, request=request)
        except VmError as e:
            e.msg = f'迁移虚拟机失败，{str(e)}'
            return self.exception_response(e)
    ...
    def vm_miss_fix(self, request, *args, **kwargs):
        ...
        try:
            VmAPI().vm_miss_fix(vm_uuid=vm_uuid, request=request)
        except VmError as e:
            return Response(data=e.data(), status=e.status_code)
    ...
    def vm_shelve(self, request, *args, **kwargs):
        ...
        try:
            api.vm_shelve(vm_uuid=vm_uuid, request=request)
        except VmError as e:
            return Response(data=e.data(), status=e.status_code)
    ...
    def vm_shelve_destroy(self, request, *args, **kwargs):
        ...
        api = VmAPI()
        try:
            api.vm_delshelve(vm_uuid=vm_uuid, request=request)
        except VmError as e:
            return Response(data=e.data(), status=e.status_code)
    ...
    def vm_attach_ip(self, request, *args, **kwargs):
        ...
        api = VmAPI()
        try:
            api.attach_ip(vm_uuid=vm_uuid, request=request, mac_ip_obj=queryset)
        except VmError as e:
            return Response(data=e.data(), status=e.status_code)
    ...
    def vm_detach_ip(self, request, *args, **kwargs):
        ...
        api = VmAPI()
        try:
            api.detach_ip(vm_uuid=vm_uuid, request=request, mac_ip_obj=queryset)
        except VmError as e:
            return Response(data=e.data(), status=e.status_code)
...
class ImageViewSet(CustomGenericViewSet):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter(
                name='mirror_image_market', in_=openapi.IN_QUERY,
                type=openapi.TYPE_BOOLEAN,
                required=False,
                description='公共镜像市场'
            ),
    ...
    def list(self, request, *args, **kwargs):
        ...
        mirror_image_market = request.query_params.get('mirror_image_market', False)
        search = request.query_params.get('search', None)

        if not mirror_image_market or mirror_image_market == 'false':
            mirror_image_market = False
        else:
        ...
        try:
            queryset = ImageManager().filter_image_queryset(center_id=center_id, sys_type=sys_type, tag=tag,
                                                            search=search, all_no_filters=True).filter(enable=True,
                                                                                                       mirror_image_market=mirror_image_market)
    ...
    @action(methods=['get'], detail=False, url_path=r'xmltemplate', url_name='image-xmltemplate')
    def get_image_xmltemplate(self, request, *args, **kwargs):
        """
        获取镜像模板信息
        ...
        queryset = ImageManager().get_xml_template()
    ...
    def get_serializer_class(self):
        ...
        if self.action == 'get_image_xmltemplate':
            return serializers.VmXmlTemplateSerializer
        return Serializer
...
class PCIDeviceViewSet(CustomGenericViewSet):
    ...
    def mount_pci(self, request, *args, **kwargs):
        ...
        user_operation_record.add_log(request=request,
                                      operation_content=f'挂载PCI设备, 云主机IP：{vm.mac_ip}, pcie设备：{device}',
                                      remark='')
    ...
    def umount_pci(self, request, *args, **kwargs):
        ...
        try:
            device = VmAPI().umount_pci_device(device_id=dev_id, user=request.user)
        except VmError as exc:
            return self.exception_response(exc)

        # 用户操作日志记录
        user_operation_record.add_log(request=request,
                                      operation_content=f'卸载PCI设备, 云主机IP:{device.vm_id}，pcie设备：{device}',
                                      remark='')
...
class VPNViewSet(CustomGenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # 用户操作日志记录
        user_operation_record.add_log(request=request, operation_content=f'创建vpn账户, VPN账户：{vpn.username}',
                                      remark='')
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        # 用户操作日志记录
        user_operation_record.add_log(request=request, operation_content=f'修改VPN账户密码, VPN账户：{vpn.username}',
                                      remark='')
    ...
    def active_vpn(self, request, *args, **kwargs):
        ...
        # 用户操作日志记录
        user_operation_record.add_log(request=request, operation_content=f'激活vpn账户, VPN账户：{vpn.username}',
                                      remark='')
        return Response(data=serializers.VPNSerializer(vpn).data, status=status.HTTP_200_OK)
    ...
    def deactive_vpn(self, request, *args, **kwargs):
        ...
        # 用户操作日志记录
        user_operation_record.add_log(request=request, operation_content=f'停用vpn账户,  VPN账户：{vpn.username}',
                                      remark='')
        return Response(data=serializers.VPNSerializer(vpn).data, status=status.HTTP_200_OK)
