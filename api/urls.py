...
router.register(r'(?P<version>(v3|v4))/pci', views.PCIDeviceViewSet, base_name='pci-device')

urlpatterns = [
    path(r'', include(router.urls)),
    re_path(r'(?P<version>(v3|v4))/token/', views.AuthTokenViewSet.as_view(), name='token'),
    re_path(r'(?P<version>(v3|v4))/jwt/', views.JWTObtainPairView.as_view(), name='jwt-token'),
    re_path(r'(?P<version>(v3|v4))/jwt-refresh/', views.JWTRefreshView.as_view(), name='jwt-refresh'),
    re_path(r'(?P<version>(v3|v4))/jwt-verify/', views.JWTVerifyView.as_view(), name='jwt-verify'),
]
